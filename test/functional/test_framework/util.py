...
def initialize_datadir(dirname, n, chain, disable_autoconnect=True):
    ...
    if not os.path.isdir(datadir):
        os.makedirs(datadir)
    write_config(os.path.join(datadir, "bitcoinblu.conf"), n=n, chain=chain, disable_autoconnect=disable_autoconnect)
...
def get_temp_default_datadir(temp_dir: pathlib.Path) -> tuple[dict, pathlib.Path]:
    ...
    if platform.system() == "Windows":
        ...
        datadir = temp_dir / "Bitcoin-Blu"
    else:
        env = dict(HOME=str(temp_dir))
        if platform.system() == "Darwin":
            datadir = temp_dir / "Library/Application Support/Bitcoin-Blu"
        else:
            datadir = temp_dir / ".bitcoin-blu"
    return env, datadir
...
def append_config(datadir, options):
    with open(os.path.join(datadir, "bitcoinblu.conf"), 'a', encoding='utf8') as f:
...
def get_auth_cookie(datadir, chain):
    user = None
    password = None
    if os.path.isfile(os.path.join(datadir, "bitcoinblu.conf")):
        with open(os.path.join(datadir, "bitcoinblu.conf"), 'r', encoding='utf8') as f:
            for line in f:
