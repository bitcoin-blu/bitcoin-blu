...
class ConfArgsTest(BitcoinTestFramework):
    ...
    def test_config_file_parser(self):
        ...
        # Check that startup fails if conf= is set in bitcoinblu.conf or in an included conf file
        ...
        with open(self.nodes[0].datadir_path / 'bitcoinblu.conf', 'a', encoding='utf-8') as conf:
        ...
        self.nodes[0].assert_start_raises_init_error(extra_args=[f"-conf={main_conf_file_path}", "-allowignoredconf"], expected_msg='Error: acceptnonstdtxn is not currently supported for main-blu chain')
    ...
    def test_config_file_log(self):
        ...
        # Write a bitcoinblu.conf file in the default data directory containing a
        # datadir= line pointing at the node datadir.
        node = self.nodes[0]
        conf_text = node.bitcoinconf.read_text()
        conf_path = default_datadir / "bitcoinblu.conf"
    ...
    def test_ignored_conf(self):
        self.log.info('Test error is triggered when the datadir in use contains a bitcoinblu.conf file that would be ignored '
        ...
        node.assert_start_raises_init_error([f"-conf={temp_conf.name}"], re.escape(
            f'Error: Data directory "{node.datadir_path}" contains a "bitcoinblu.conf" file which is ignored, because a '
        ...
        # the same bitcoinblu.conf that would be loaded anyway does not trigger an
        # error.
        self.start_node(0, [f'-conf={node.datadir_path}/bitcoinblu.conf'])
        self.stop_node(0)
    ...
    def test_ignored_default_conf(self):
        ...
        self.log.info('Test error is triggered when bitcoinblu.conf in the default data directory sets another datadir '
                      'and it contains a different bitcoinblu.conf file that would be ignored')
        ...
        # bitcoinblu.conf that would be ignored.
        node = self.nodes[0]
        (default_datadir / "bitcoinblu.conf").write_text(f"datadir={node.datadir_path}\n")
        ...
        node.assert_start_raises_init_error([], re.escape(
            f'Error: Data directory "{node.datadir_path}" contains a "bitcoinblu.conf" file which is ignored, because a '
            f'different configuration file "{default_datadir}/bitcoinblu.conf" from data directory "{default_datadir}" '
    ...
    def test_acceptstalefeeestimates_arg_support(self):
        ...
        conf_file = self.nodes[0].datadir_path / "bitcoinblu.conf"
        for chain, chain_name in {("main-blu", ""), ("test", "testnet3"), ("signet", "signet"), ("testnet4", "testnet4")}:
    ...
    def run_test(self):
        ...
        conf_file = default_data_dir / "bitcoinblu.conf"
        ...
        none_existent_conf_file = default_data_dir / "none_existent_bitcoinblu.conf"
