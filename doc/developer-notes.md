...
configuration options and toggled while bitcoinblu-d is running with the `logging`
RPC.  For instance, launching bitcoinblu-d with `-debug` or `-debug=1` will turn on
...
i.e. `CPPFLAGS="-DDEBUG_LOCKCONTENTION"`, then build and run bitcoinblu-d.

You can then use the `-debug=lock` configuration option at bitcoinblu-d startup or
`bitcoinblu-cli logging '["lock"]'` at runtime to turn on lock contention logging.
It can be toggled off again with `bitcoinblu-cli logging [] '["lock"]'`.
...
$ valgrind -v --leak-check=full src/bitcoinblu-d -printtoconsole
...
To profile a running bitcoinblu-d process for 60 seconds, you could use an
invocation of `perf record` like this:

```sh
$ perf record \
    -g --call-graph dwarf --per-thread -F 140 \
    -p `pgrep bitcoinblu-d` -- sleep 60
```
...
- [Main thread (`bitcoinblu-d`)](https://doxygen.bitcoincore.org/bitcoind_8cpp.html#a0ddf1224851353fc92bfbff6f499fa97)
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `bitcoinblu-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `bitcoinblu-cli` and the GUI debug console use this table to determine how to
