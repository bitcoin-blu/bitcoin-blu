...
It is possible to specify `bitcoinblu-d` arguments to the `fuzz` executable.
...
Core without having to write any custom fuzzing harness. The `bitcoinblu-d` server
...
EOF
$ make -C src/ bitcoinblu-d
$ mkdir -p inputs/
$ honggfuzz/honggfuzz --exit_upon_crash --quiet --timeout 4 -n 1 -Q \
      -E HFND_TCP_PORT=18454 -f inputs/ -- \
          src/bitcoinblu-d -regtest -discover=0 -dns=0 -dnsseed=0 -listenonion=0 \
                       -nodebuglogfile -bind=127.0.0.1:18454 -logthreadnames \
                       -debug
```
