...
- When the `-port` configuration option is used, the default onion listening port will now
  be derived to be that port + 1 instead of being set to a fixed value (8344 on mainnet).
  ...
  previously Bitcoin Core would listen for incoming Tor connections on `127.0.0.1:8344`.
  Now it would listen on `127.0.0.1:5556` (`-port` plus one). If you configured the hidden service manually
  in torrc now you have to change it from `HiddenServicePort 8343 127.0.0.1:8344` to `HiddenServicePort 8343
  127.0.0.1:5556`, or configure bitcoind with `-bind=127.0.0.1:8344=onion` to get the previous behavior.
  (#31223)
