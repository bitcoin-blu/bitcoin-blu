...
static RPCHelpMan addnode()
{
    return RPCHelpMan{"addnode",
                ...
                RPCResult{RPCResult::Type::NONE, "", ""},
                RPCExamples{
                    HelpExampleCli("addnode", "\"192.168.0.6:8343\" \"onetry\" true")
            + HelpExampleRpc("addnode", "\"192.168.0.6:8343\", \"onetry\" true")
                },
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
...
static RPCHelpMan addconnection()
{
    return RPCHelpMan{"addconnection",
        ...
        RPCExamples{
            HelpExampleCli("addconnection", "\"192.168.0.6:8343\" \"outbound-full-relay\" true")
            + HelpExampleRpc("addconnection", "\"192.168.0.6:8343\" \"outbound-full-relay\" true")
        },
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
...
static RPCHelpMan addpeeraddress()
{
    return RPCHelpMan{"addpeeraddress",
        ...
        RPCExamples{
            HelpExampleCli("addpeeraddress", "\"1.2.3.4\" 8343 true")
    + HelpExampleRpc("addpeeraddress", "\"1.2.3.4\", 8343, true")
        },
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
...
static RPCHelpMan getbbluinfo()
{
    return RPCHelpMan{
        "getbbluinfo",
        "Request bbluinfo from a peer and return the response.\n"
        ...
        RPCResult{
            RPCResult::Type::OBJ, "", "",
            {
                ...
                {RPCResult::Type::STR, "bbluinfo", "Bbluinfo string received from peer (empty if not received or timeout)"},
            }
        },
        RPCExamples{
            HelpExampleCli("getbbluinfo", "\"192.168.0.6:8343\"")
            + HelpExampleRpc("getbbluinfo", "\"192.168.0.6:8343\"")
        },
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
        {
            NodeContext& node = EnsureAnyNodeContext(request.context);
            ...
            // Send bbluinfo request (empty string)
            bool found = connman.ForNode(nodeid, [&](CNode* pnode) {
                CSerializedNetMsg msg = NetMsg::Make(NetMsgType::BBLUINFO, std::string{});
