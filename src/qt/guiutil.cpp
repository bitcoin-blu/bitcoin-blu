...
static std::string DummyAddress(const CChainParams &params)
{
    ...
    case ChainType::REGTEST:
        addr = "bbrt1p6my3utxpm6q6gk96tyveaz8udye9t0a8wl9hwynhmhxwgw3j94rqancc27";
        break;
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Bitcoin-BBLU address (e.g. %1)").arg(
...
{
    // return if URI is not valid or is no bitcoin-blu: URI
    if(!uri.isValid() || uri.scheme() != QString("bitcoin-blu"))
        return false;
...
QString formatBitcoinURI(const SendCoinsRecipient &info)
{
    ...
    QString ret = QString("bitcoin-blu:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;
...
bool openBitcoinConf()
{
    ...
    /* Open bitcoinblu.conf with the associated application */
