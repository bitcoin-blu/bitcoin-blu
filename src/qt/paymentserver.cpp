...
const QString BITCOIN_IPC_PREFIX("bitcoin-blu:");
...
void PaymentServer::ipcParseCommandLine(int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        if (arg.startsWith(BITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // bitcoin-blu: URI
        {
    ...
    : QObject(parent)
{
    ...
    // on Mac: sent when you click bitcoin-blu: links
    ...
    if (startLocalServer)
    {
        ...
        if (!uriServer->listen(name)) {
            ...
            QMessageBox::critical(nullptr, tr("Payment request error"),
                tr("Cannot start bitcoin-blu: click-to-pay handler"));
        }
        else {
...
//
// OSX-specific way of handling bitcoin-blu: URIs
//
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith("bitcoin-blu://", Qt::CaseInsensitive))
    {
        Q_EMIT message(tr("URI handling"), tr("'bitcoin-blu://' is not a valid URI. Use 'bitcoin-blu:' instead."),
            CClientUIInterface::MSG_ERROR);
    }
    else if (s.startsWith(BITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // bitcoin-blu: URI
    {
